...
A critical performance optimization in this pipeline is **Graph Compilation Caching**. The LangGraph instance is compiled exactly once during application initialization and cached within the `GraphRAGRetrievalUseCase`. Per-request parameters like `top_k` and `notebook_ids` are passed through the graph's internal state, eliminating the per-request overhead of graph re-compilation and significantly reducing overall retrieval latency.
