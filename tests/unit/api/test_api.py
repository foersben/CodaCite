...
from app.models.models import Document, Notebook


@pytest.fixture(autouse=True)
def mock_dependency_overrides(mocker: Any) -> None:
    """Mock heavy dependency providers to avoid hangs and network calls."""
    from app.api.dependencies import (
        get_coref_resolver,
        get_document_store,
        get_embedder,
    ...
    app.dependency_overrides[get_generator] = lambda: mocker.MagicMock()
    app.dependency_overrides[get_vlm] = lambda: mocker.MagicMock()
    app.dependency_overrides[get_embedder] = lambda: mocker.MagicMock()
    app.dependency_overrides[get_coref_resolver] = lambda: mocker.MagicMock()
    app.dependency_overrides[get_document_store] = lambda: mocker.MagicMock()
    app.dependency_overrides[get_extractor] = lambda: mocker.MagicMock()
...
def client(mocker: Any) -> Generator[TestClient]:
    ...
    We mock ensure_models_exist to prevent it from trying to download
    models from HuggingFace during bootstrap.
    """
    mocker.patch("app.main.init_db", new_callable=mocker.AsyncMock)
    mocker.patch("app.main.ensure_models_exist")
    with TestClient(app) as c:
        yield c


def test_root_endpoint(mocker: Any, client: TestClient) -> None:
...
def test_notebook_management_endpoints(mocker: Any, client: TestClient) -> None:
...
def test_document_notebook_relation_endpoints(mocker: Any, client: TestClient) -> None:
...
def test_document_management_endpoints(mocker: Any, client: TestClient) -> None:
...
def test_query_endpoint(mocker: Any, client: TestClient) -> None:
    ...
    mock_retrieval.execute = mocker.AsyncMock(return_value={"documents": [{"text": "Result"}]})
...
def test_chat_endpoint(mocker: Any, client: TestClient) -> None:
...
def test_ingest_endpoint(mocker: Any, client: TestClient) -> None:
    ...
    mocker.patch("app.api.routes.DocumentLoader.load", return_value=[mock_doc])
...
def test_ingest_no_filename(client: TestClient) -> None:
...
def test_ingest_unsupported_format(mocker: Any, client: TestClient) -> None:
    ...
    Args:
        ...
        client: Test client.
    """
    mocker.patch("app.api.routes.DocumentLoader.load", side_effect=ValueError("Format error"))
    file = {"file": ("test.exe", io.BytesIO(b"data"), "application/octet-stream")}
    response = client.post("/api/v1/ingest", files=file)
    assert response.status_code == 400


def test_ingest_unexpected_error(mocker: Any, client: TestClient) -> None:
    ...
    Args:
        ...
        client: Test client.
    """
    mocker.patch("app.api.routes.DocumentLoader.load", side_effect=Exception("Disk full"))
    file = {"file": ("test.txt", io.BytesIO(b"data"), "text/plain")}
    response = client.post("/api/v1/ingest", files=file)
    assert response.status_code == 500


def test_get_document_status_endpoints(mocker: Any, client: TestClient) -> None:
...
def test_notebook_ui(mocker: Any, client: TestClient) -> None:
    ...
    mocker.patch(
        "app.api.routes.templates.TemplateResponse",
...
def test_get_notebook_documents_endpoint(mocker: Any, client: TestClient) -> None:
