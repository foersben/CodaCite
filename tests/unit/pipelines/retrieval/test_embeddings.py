...
def embedder(mocker: Any, mock_tokenizer: Any, mock_model: Any) -> HuggingFaceEmbedder:
    ...
    """
    mocker.patch(
        "app.pipelines.retrieval.embeddings.AutoTokenizer.from_pretrained",
        return_value=mock_tokenizer,
    )
    mocker.patch(
        "app.pipelines.retrieval.embeddings.AutoModel.from_pretrained",
        return_value=mock_model,
    )

    # Force standard model initialization via settings mocks
    # Patch app.core.config.settings because it is imported inside __init__
...
async def test_torch_quantization_init(mocker: Any, mock_tokenizer: Any) -> None:
    ...
    mocker.patch("app.pipelines.retrieval.embeddings.AutoModel.from_pretrained")
...
async def test_init_standard_fallback_on_device_error(mocker: Any, mock_tokenizer: Any) -> None:
    ...
    mocker.patch(
        "app.pipelines.retrieval.embeddings.AutoModel.from_pretrained", return_value=mock_model_obj
    )
...
async def test_quantization_caching(mocker: Any, mock_tokenizer: Any) -> None:
    ...
    mock_load = mocker.patch(
        "app.pipelines.retrieval.embeddings.torch.load", return_value=mock_model_obj
    )
...
async def test_openvino_init_failure(mocker: Any, mock_tokenizer: Any) -> None:
    ...
    mock_standard_init = mocker.patch(
        "app.pipelines.retrieval.embeddings.HuggingFaceEmbedder._init_standard_model"
    )
