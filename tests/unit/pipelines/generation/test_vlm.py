...
def mock_llama(mocker: Any) -> MagicMock:
    ...
    mock_class = mocker.patch("app.pipelines.generation.vlm.Llama")
    return mock_class


def test_vlm_init_no_path(mocker: Any) -> None:
    """Tests VLM initialization when no model path is configured."""
    mocker.patch("app.pipelines.generation.vlm.settings.local_vlm_path", "")
    vlm = LocalVLM()
...
def test_vlm_init_path_not_exists(mocker: Any) -> None:
    ...
    mocker.patch("app.pipelines.generation.vlm.settings.local_vlm_path", "/non/existent/path")
    vlm = LocalVLM()
    assert vlm.llm is None
...
def test_vlm_init_success(mocker: Any, tmp_path: Path, mock_llama: MagicMock) -> None:
    ...
    mocker.patch("app.pipelines.generation.vlm.settings.local_vlm_path", str(model_file))
    vlm = LocalVLM()
