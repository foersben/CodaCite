...
def ensure_models_exist() -> None:
    ...
    if not emb_dir.exists() or not any(emb_dir.iterdir()):
        logger.info(
            "[Bootstrap] Initializing download for Embedding Model: %s", settings.embedding_model_id
        )
        try:
            snapshot_download(
                repo_id=settings.embedding_model_id,
    ...
    if not llm_path.exists():
        logger.info(
            "[Bootstrap] Initializing download for Generative AI Model: %s (~4.8GB)", llm_filename
        )
